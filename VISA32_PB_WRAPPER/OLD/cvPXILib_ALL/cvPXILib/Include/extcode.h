...
#define Hi32(x)			((int32)((int64)(x)>>32))
#define Lo32(x)			((int32)(int64)(x))
...
#define Cat8Chrs(c1,c2,c3,c4, c5, c6, c7, c8)	(((int64)(uInt8)(c1)<<56)|((int64)(uInt8)(c2)<<48)|((int64)(uInt8)(c3)<<40)|((int64)(uInt8)(c4)<<32)|\
												((int64)(uInt8)(c5)<<24)|((int64)(uInt8)(c6)<<16)|((int64)(uInt8)(c7)<<8)|((int64)(uInt8)(c8)))

#if NI_BIG_ENDIAN
	#define RTToL(c1,c2,c3,c4)	Cat4Chrs(c1,c2,c3,c4)
	#define RTToW(c1,c2)		Cat2Chrs(c1,c2)
	#define IntFrom4Chars(s)	((int32)s)
#else
	#define RTToL(c1,c2,c3,c4)	Cat4Chrs(c4,c3,c2,c1)
	#define RTToW(c1,c2)		Cat2Chrs(c2,c1)
	#define IntFrom4Chars(s)	(((int32)(s)<<24)|(((int32)(s)&0x0000ff00)<<8)|((uInt32)(s)>>24)|(((int32)(s)&0x00ff0000)>>8))
#endif

/* Using 1 instead of 0 avoids a warning in GCC */
#define Offset(type, field)		(((size_t)(&((type*)1)->field))-1)
...
TH_REENTRANT int32 _FUNCC	PtrLenStrCmp(const uChar*, int32, const uChar*, int32);
TH_REENTRANT int32 _FUNCC	PtrLenStrCaseCmp(const uChar*, int32, const uChar*, int32);
...
TH_REENTRANT int32 _FUNCC	BlockCmp(const void * p1, const void * p2, size_t n);
...
typedef struct {
	...
	size_t totAllocSize, maxAllocSize;
...
#define kNotAMagicCookie ((MagicCookie)0)	/* canonical invalid magic cookie */
...
/**
Enumerated type for file types.
The enumeration is used to increase type safety but does not list all possible
values. Other values may be explicitly cast to this enumeration type.
*/
...
/**
Enumerated type for file creator codes.
The enumeration is used to increase type safety but does not list all possible
values. Other values may be explicitly cast to this enumeration type.
*/
typedef enum  {
...
TH_REENTRANT MgErr _FUNCC	FCreate(File *fdp, ConstPath path, int32 access, int32 openMode, int32 denyMode, PStr group);
TH_REENTRANT MgErr _FUNCC	FCreateAlways(File *fdp, ConstPath path, int32 access, int32 openMode, int32 denyMode, PStr group);
TH_REENTRANT MgErr _FUNCC	FMOpen(File *fdp, ConstPath path, int32 openMode, int32 denyMode);
...
TH_REENTRANT MgErr _FUNCC	FGetInfo(ConstPath path, FInfoPtr infop);
TH_REENTRANT MgErr _FUNCC	FGetInfo64(ConstPath path, FInfo64Ptr infop, FGetInfoWhich which DEFAULTARG(kFGetInfoAll));
TH_REENTRANT int32 _FUNCC	FExists(ConstPath path);
TH_REENTRANT MgErr _FUNCC	FGetAccessRights(ConstPath path, PStr owner, PStr group, int32 *permPtr);
TH_REENTRANT MgErr _FUNCC	FSetInfo(ConstPath path, FInfoPtr infop);
TH_REENTRANT MgErr _FUNCC	FSetInfo64(ConstPath path, FInfo64Ptr infop);
TH_REENTRANT MgErr _FUNCC	FSetAccessRights(ConstPath path, PStr owner, PStr group, int32 *permPtr);
TH_REENTRANT MgErr _FUNCC	FMove(ConstPath oldPath, ConstPath newPath);
TH_REENTRANT MgErr _FUNCC	FCopy(ConstPath oldPath, ConstPath newPath);
TH_REENTRANT MgErr _FUNCC	FRemove(ConstPath path);
TH_REENTRANT MgErr _FUNCC	FRemoveToRecycle(ConstPath path, Bool32 showConfirmDlgs);
TH_REENTRANT MgErr _FUNCC	FNewDir(ConstPath path, int32 permissions);

typedef CPStr FDirEntRec, *FDirEntPtr, **FDirEntHandle; /**< directory list record */

TH_REENTRANT MgErr _FUNCC	FListDir(ConstPath path, FDirEntHandle list, FMListDetails **);
TH_REENTRANT MgErr _FUNCC	FAddPath(ConstPath basePath, ConstPath relPath, Path newPath);
TH_REENTRANT MgErr _FUNCC	FAppendName(Path path, ConstPStr name);
TH_REENTRANT MgErr _FUNCC	FRelPath(ConstPath start, ConstPath end, Path relPath);
TH_REENTRANT MgErr _FUNCC	FName(ConstPath path, StringHandle name);
TH_REENTRANT MgErr _FUNCC	FNamePtr(ConstPath path, PStr name);
TH_REENTRANT MgErr _FUNCC	FDirName(ConstPath path, Path dir);
TH_REENTRANT MgErr _FUNCC	FVolName(ConstPath path, Path vol);
...
TH_REENTRANT MgErr _FUNCC	FPathCpy(Path dst, ConstPath src);

// FDestroyPath releases memory AND sets pointer to NULL.
TH_REENTRANT void _FUNCC	FDestroyPath(Path *pp);
// Use of FDestroyPath recommended over FDisposePath.
TH_REENTRANT MgErr _FUNCC	FDisposePath(Path p);

TH_REENTRANT int32 _FUNCC	FUnFlattenPath(UPtr fp, Path *pPtr);
TH_REENTRANT int32 _FUNCC	FFlattenPath(ConstPath p, UPtr fp);
TH_REENTRANT int32 _FUNCC	FDepth(ConstPath path);
...
TH_REENTRANT int32 _FUNCC	FPathCmp(ConstPath, ConstPath);
TH_REENTRANT int32 _FUNCC	FPathCmpLexical(ConstPath, ConstPath);
TH_REENTRANT UHandle _FUNCC	PathToCString(ConstPath );
TH_REENTRANT MgErr _FUNCC	FPathToDSString(ConstPath, LStrHandle*);
TH_REENTRANT MgErr _FUNCC	FStringToPath(ConstLStrH, Path*);
TH_REENTRANT MgErr _FUNCC	FPathToFileSystemDSString(ConstPath p, LStrHandle *txt);
...
TH_REENTRANT MgErr _FUNCC	FMGetVolInfo(ConstPath, float64*, float64*);
TH_REENTRANT MgErr _FUNCC	FMGetVolInfo64(ConstPath, uInt64*, uInt64*, uInt32*);
TH_REENTRANT MgErr _FUNCC	FMGetVolPath(ConstPath, Path*);
TH_REENTRANT MgErr _FUNCC	FSetPathType(Path, int32);
TH_REENTRANT MgErr _FUNCC	FGetPathType(ConstPath, int32*);
TH_REENTRANT Bool32 _FUNCC	FIsAPath(ConstPath);
TH_REENTRANT Bool32 _FUNCC	FIsEmptyPath(ConstPath);
TH_REENTRANT Bool32 _FUNCC	FIsAPathOrNotAPath(ConstPath);
TH_REENTRANT Bool32 _FUNCC	FIsAPathOfType(ConstPath, int32);
...
TH_REENTRANT MgErr _FUNCC	FNewRefNum(ConstPath, File, LVRefNum*);
...
TH_REENTRANT MgErr _FUNCC	FPathToArr(ConstPath, Bool32*, UHandle);
TH_REENTRANT int32 _FUNCC	FPrintf(File, ConstCStr, ...);  /* moved from support manager area */
TH_REENTRANT MgErr _FUNCC	FPrintfWErr(File fd, ConstCStr fmt, ...);
...
#define UseDefaultCINDispose CIN MgErr _FUNCC CINDispose() { return noErr; }
...
#define UseDefaultCINLoad CIN MgErr _FUNCC CINLoad(RsrcFile reserved) { Unused(reserved); return noErr; }

/* CINUnload -- Called when the VI is unloaded. */
#define UseDefaultCINUnload CIN MgErr _FUNCC CINUnload() { return noErr; }

/* CINSave -- Called when the VI is saved. */
#define UseDefaultCINSave CIN MgErr _FUNCC CINSave(RsrcFile reserved) { Unused(reserved); return noErr; }
