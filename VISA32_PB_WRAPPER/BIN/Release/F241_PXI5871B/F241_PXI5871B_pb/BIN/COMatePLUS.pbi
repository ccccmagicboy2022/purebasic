...
;*======================================
;*   Stephen Rodriguez (srod) with thanks to ts-soft, kiffi, mk-soft.
...
;-DECLARES.
  ...
  CompilerIf Defined(INCLUDE_FREESTRINGLIB, #PB_Constant)=0
    Declare FreeStructureString(*Address)
  CompilerEndIf
        ...
        For i = 0 To COMate_gNumThreadElements-1
          *error = PeekI(COMate_gPtrThreadArray + i*SizeOf(i))
          If *error ;Just in case!
            FreeStructureString(@*error\lastError$)
            FreeMemory(*error)
          EndIf
        Next
...
Procedure COMateClass_INTERNAL_SetError(*this._membersCOMateClass, result, blnAllowDispError = 0, dispError$="")
  Protected *error._COMateThreadErrors, Array.i
